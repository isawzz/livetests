...
function autopoll(ms) {
	...
	if (Pollmode == 'auto' 
	&& isdef(Z) 
	&& (Z.role == 'spectator' || Z.mode == 'multi' && (Z.turn.length > 1 || is_shield_mode() || Z.fen.keeppolling == true))) {
...
function _poll() {
	...
	//Counter.poll = isdef(Counter.poll) ? Counter.poll + 1 : 1; console.log('____poll:', Counter.poll);
