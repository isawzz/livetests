...
if ($cmd == "assets") {
  ...
  $result->table = $table;
	...
	//notes soll ein akku sein! es sammelt
  ...
  $table = db_read($qr)[0]; 
	...
	if ($astep == $estep && $astage == $estage){
		...
		$modified = get_now();
		$qw = "UPDATE gametable SET `fen`='$fen',`expected`='$expected',`phase`='$phase',`round`=$round,`step`=$step,`stage`='$stage',`action`='$action',modified=$modified,`notes`='$notes' WHERE `friendly` = '$friendly'"; //ok
		$qr="SELECT * FROM gametable WHERE `friendly` = '$friendly' limit 1";
